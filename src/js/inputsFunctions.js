...
const bmiDescriptionActive = document.querySelector(
  ".div-bmi-card-descripion-active"
);
...
export function inputImperialHeightFootFun() {
  ...
  showBmiActive();
  const heightFootInMeters =
    (Number(inputImperialHeightFoot.value) * 30.5) / 100;
  const heightInchesInMeters =
    (Number(inputImperialHeightInch.value) * 2.55) / 100;
...
export function inputImperialWeightStoneFun() {
  if (
    inputImperialHeightFoot.value == "" &&
    inputImperialHeightInch.value == ""
  ) {
    hideBmiActive();
    return;
  }
